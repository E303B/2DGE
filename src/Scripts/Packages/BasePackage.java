    ...
    public final boolean tryRun(String command, Object scriptsParams, ScriptRunner runner) {
        if (childFunctions != null) {
            for (BaseFunction function : childFunctions) {
                if (command.startsWith(function.getClass().getName())) {
                    function.run(command.substring(function.getClass().getName().length() + 1).trim(), scriptsParams,
                            runner);
                    return true;
                }
            }
        ...
        if (childPackages != null)
            for (BasePackage childPackage : childPackages)
                if (command.startsWith(childPackage.getClass().getName()))
                    return childPackage.tryRun(command.substring(childPackage.getClass().getName().length() + 2).trim(),
